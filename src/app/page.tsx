...
function StudentLogin() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const sessionSnap = await getDoc(doc(db, sessionPath(code)));
      if (!sessionSnap.exists()) {
        setError("존재하지 않는 세션 코드예요. 선생님께 확인해주세요.");
        setLoading(false);
        return;
      }

      const studentRef = doc(db, studentPath(code, no));
      ...
      saveStudentAuth({ sessionCode: code, studentId: no, name: trimmedName });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="mb-1 block text-sm font-bold text-slate-800">세션 코드</label>
        <Input
          value={sessionCode}
...
function TeacherLogin() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      if (!data.ok) {
        setError(data.error ?? "세션 생성에 실패했어요.");
        setLoading(false);
        return;
      }
      saveTeacherPin(data.sessionCode, pin);
      router.push(`/teacher/${data.sessionCode}`);
    } catch (err) {
      console.error(err);
      setError("세션 생성 중 문제가 발생했어요.");
      setLoading(false);
    }
  }
  ...
  async function handleJoin(e: React.FormEvent) {
    ...
    try {
      ...
      const sessionSnap = await getDoc(doc(db, sessionPath(code)));
      if (!sessionSnap.exists()) {
        setError("존재하지 않는 세션 코드예요.");
        setLoading(false);
        return;
      }
      saveTeacherPin(code, pin);
  ...
  return (
    <div className="space-y-4">
      <div className="inline-flex w-full rounded-full bg-slate-100 p-1 text-xs font-bold">
        <button
          onClick={() => setMode("join")}
          className={`flex-1 rounded-full px-3 py-1.5 ${mode === "join" ? "bg-white text-rose-600 shadow" : "text-slate-500"}`}
        >
          기존 세션 입장
        </button>
        <button
          onClick={() => setMode("create")}
          className={`flex-1 rounded-full px-3 py-1.5 ${mode === "create" ? "bg-white text-rose-600 shadow" : "text-slate-500"}`}
        >
          새 연구소 개소
        </button>
      </div>
      ...
      {mode === "join" ? (
        <form onSubmit={handleJoin} className="space-y-4">
          ...
          <div>
            ...
            <Input type="password" value={pin} onChange={(e) => setPin(e.target.value)} placeholder="교사 PIN" />
          </div>
          {error && <p className="text-sm font-bold text-red-600">{error}</p>}
          <Button type="submit" disabled={loading} className="w-full">
        ...
        <form onSubmit={handleCreate} className="space-y-4">
          <p className="text-sm text-slate-500">새 세션 코드를 발급하고 공식 의뢰 15건을 자동으로 등록해요.</p>
          <div>
            <label className="mb-1 block text-sm font-bold text-slate-800">PIN</label>
            <Input type="password" value={pin} onChange={(e) => setPin(e.target.value)} placeholder="교사 PIN" />
          </div>
